...
public class Matrices
{
    ...
    public int[] FindDiagonalOrder(int[][] mat)
    {
        ...
        for (int i = 0; i < startPos.GetLength(0); i++)
        {
            if (i % 2 == 0)
            {
                startPos[i, 0] = Math.Min(i, bottom);
                startPos[i, 1] = Math.Max(0, i - bottom);
            }
            else
            {
                startPos[i, 0] = Math.Max(0, i - right);
                startPos[i, 1] = Math.Min(i, right);
            }
        }
